...
function renderHero(feed) {
  const boston = feed.away.id === 111 ? feed.away : feed.home;
  const opponent = feed.away.id === 111 ? feed.home : feed.away;
  document.getElementById('gameKicker').textContent = `${gameDate(feed.game_date)} · ${feed.venue} · Final`;
  document.getElementById('gameResult').textContent = `${feed.result} · Boston ${boston.record}`;
  document.getElementById('gameSummary').textContent = feed.summary;

  const decisions = document.getElementById('gameDecisions');
  const pieces = [];
  if (feed.decisions.winner) pieces.push(`W · ${feed.decisions.winner}`);
  if (feed.decisions.loser) pieces.push(`L · ${feed.decisions.loser}`);
  if (feed.decisions.save) pieces.push(`SV · ${feed.decisions.save}`);
  decisions.replaceChildren(...pieces.map(text => {
  ...
  [feed.away, feed.home].forEach(team => {
    ...
    row.className = `score-team${team.id === 111 ? ' boston' : ''}`;
    const identity = document.createElement('div');
    addText(identity, 'span', 'score-abbr', team.abbreviation);
    addText(identity, 'span', 'score-name', team.name);
    row.appendChild(identity);
    addText(row, 'strong', 'score-runs', team.runs);
    score.appendChild(row);
  });
  addText(score, 'div', 'score-final-label', `Final · ${feed.innings_count} innings`);
  gameHero.hidden = false;
}

function renderDetails(feed) {
  const facts = document.getElementById('gameFacts');
  facts.replaceChildren(...feed.facts.map(text => {
    const item = document.createElement('li');
    item.textContent = text;
    return item;
  }));

  const recap = document.getElementById('officialRecap');
  if (feed.official_recap?.url && feed.official_recap?.headline) {
    recap.href = feed.official_recap.url;
    recap.textContent = `MLB recap · ${feed.official_recap.headline} ↗`;
    recap.hidden = false;
  }
...
async function loadGame() {
  try {
    ...
    console.error(error);
    status.textContent = 'The most recent game could not be loaded right now. Please try again soon.';
  }
}
