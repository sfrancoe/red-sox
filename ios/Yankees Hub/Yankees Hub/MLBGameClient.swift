...
struct MLBGameClient: Sendable {
    private static let teamID = TeamConfig.teamID
    ...
    private func buildGame(from payload: JSON) throws -> RecentGame {
        ...
        guard away.id == Self.teamID || home.id == Self.teamID else {
            throw MLBGameError.notFavoriteTeam
        }
        let favorite = away.id == Self.teamID ? away : home
        let opponent = away.id == Self.teamID ? home : away
    ...
    private func liveSummary(
        ...
        if favorite.runs > opponent.runs {
            return "The Yankees lead the \(clubName(opponent)), \(favorite.runs)–\(opponent.runs), \(situation) at \(venue)."
        }
        if favorite.runs < opponent.runs {
            return "The Yankees trail the \(clubName(opponent)), \(opponent.runs)–\(favorite.runs), \(situation) at \(venue)."
        }
        return "The Yankees and \(clubName(opponent)) are tied, \(favorite.runs)–\(opponent.runs), \(situation) at \(venue)."
    }
    ...
    private func finalSummary(
            ...
            if walkoff, let winningPlay {
                sentences.append(
                    "\(winningPlay.play.batter) delivered a walk-off \(winningPlay.play.event.lowercased()) "
                        + "in the \(ordinal(winningPlay.play.inningNum)) inning as the Yankees rallied past "
                        + "the \(clubName(opponent)), \(favorite.runs)–\(opponent.runs), at \(venue)."
                )
            } else if largestDeficit >= 2 {
                sentences.append(
                    "The Yankees erased a \(largestDeficit)-run deficit to beat the \(clubName(opponent)), "
                        + "\(favorite.runs)–\(opponent.runs), at \(venue)."
                )
            } else if opponent.runs == 0 {
                sentences.append("The Yankees shut out the \(clubName(opponent)), \(favorite.runs)–\(opponent.runs), at \(venue).")
            } else {
                sentences.append("The Yankees beat the \(clubName(opponent)), \(favorite.runs)–\(opponent.runs), at \(venue).")
            }
            if largestDeficit >= 2, annotated.indices.contains(deficitIndex) {
                ...
                if let rally = annotated.dropFirst(deficitIndex + 1).first(where: {
                    ...
                    let effect = remaining == 0 ? "tied the game"
                        : remaining < 0 ? "put New York ahead"
                        : "cut the deficit to \(remaining == 1 ? "one" : "\(remaining)")"
                    sentences.append(
                        "New York trailed \(lowPoint.afterOpponent)–\(lowPoint.afterFavorite) before "
        ...
        var sentences: [String] = []
        if favorite.runs == 0 {
            return "The Yankees were shut out by the \(clubName(opponent)), \(opponent.runs)–\(favorite.runs), at \(venue)."
        }
        if largestLead >= 2 {
            sentences.append(
                "The Yankees couldn’t hold a \(largestLead)-run lead and fell to the \(clubName(opponent)), "
                    + "\(opponent.runs)–\(favorite.runs), at \(venue)."
            )
        } else {
            sentences.append(
                "The Yankees fell to the \(clubName(opponent)), \(opponent.runs)–\(favorite.runs), at \(venue)."
            )
        }
        ...
        if let favoriteHighlight,
           favoriteHighlight.play.id != opponentGoAhead?.play.id {
            sentences.append(
                "New York’s biggest swing came on \(scoringAction(favoriteHighlight.play)) "
    ...
    private func interestingFacts(
        ...
        if let top = favorite.batting.max(by: { $0.hits < $1.hits }), top.hits >= 2 {
            facts.append("\(top.name) has \(top.hits) of the Yankees’ \(favorite.hits) hits\(isLive ? " so far" : "").")
        }
        ...
        if !homers.isEmpty {
            ...
            facts.append("The Yankees have hit \(total) home run\(total == 1 ? "" : "s"): \(names).")
        }
