...
struct PitcherReport: Codable, Identifiable, Sendable {
    ...
    var story: String {
        ...
        if warGap >= 0.5 {
            return "\(warGap.signed(places: 1)) fWAR ahead of forecast while covering \(inningsSharePct.formatted(.number.precision(.fractionLength(1))))% of New York’s innings."
        }
        if warGap >= 0.15 {
            return "Quietly ahead of plan: \(warGap.signed(places: 1)) fWAR, plus \(actual.ip) innings that helped hold the staff together."
        }
        if warGap > -0.15 {
            return "Almost exactly on the value forecast so far, accounting for \(inningsSharePct.formatted(.number.precision(.fractionLength(1))))% of New York’s innings."
        }
