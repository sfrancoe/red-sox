#!/usr/bin/env python3
"""Fetch the upcoming Mets schedule from the MLB Stats API."""
...
def game_row(game: dict[str, Any], today, pitcher_records: dict[int, str]) -> dict[str, Any] | None:
    ...
    if away_team.get("id") == NYM:
        favorite_team, opponent, location = away, home, "away"
    elif home_team.get("id") == NYM:
...
def main() -> None:
    ...
    url = API.format(team=NYM, start=today.isoformat(), end=end.isoformat())
    ...
    print(f"Wrote {len(feed['games'])} upcoming Mets games to {OUTPUT_PATH}")
