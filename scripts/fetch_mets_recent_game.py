#!/usr/bin/env python3
"""Fetch the most recent completed Mets game and build a compact recap."""
...
OUTPUT_PATH = ROOT / "data" / "mets" / "recent-game.json"
SCHEDULE_API = (
...
def latest_final_game(today) -> int:
    ...
    url = SCHEDULE_API.format(team=NYM, start=start.isoformat(), end=today.isoformat())
    ...
    for date_entry in payload.get("dates", []):
        for game in date_entry.get("games", []):
            if (game.get("status") or {}).get("abstractGameState") == "Final":
                finals.append(game)
    if not finals:
        raise RuntimeError("No completed Mets game found in the past 14 days")
...
def scoring_action(play: dict[str, Any]) -> str:
    batter = play.get("batter") or "New York"
...
def build_summary(boston: dict[str, Any], opponent: dict[str, Any], venue: str,
    ...
    if br > oruns:
        ...
        if walkoff:
            first = (
                f"{winning_play.get('batter') or 'New York'} delivered a walk-off "
                f"{str(winning_play.get('event') or 'hit').lower()} in the "
                f"{ordinal(int(winning_play['inning_num']))} inning as the Mets rallied past "
                f"the {opponent_name}, {score}, at {venue}."
            )
        elif largest_deficit >= 2:
            first = (
                f"The Mets erased a {largest_deficit}-run deficit to beat the "
                f"{opponent_name}, {score}, at {venue}."
            )
        elif oruns == 0:
            first = f"The Mets shut out the {opponent_name}, {score}, at {venue}."
        else:
            first = f"The Mets beat the {opponent_name}, {score}, at {venue}."
        ...
        if largest_deficit >= 2 and annotated:
            ...
            if rally_play:
                ...
                effect = (
                    ...
                    else "put New York ahead" if remaining < 0
                    else f"cut the deficit to {'one' if remaining == 1 else remaining}"
                )
                details.append(
                    f"New York trailed {low_point['after_opponent']}–{low_point['after_boston']} before "
        ...
        if walkoff and tying_play and tying_play is not winning_play:
            ...
            details.append(
                ...
                f"{winning_play.get('batter') or 'New York'} completed the comeback "
                f"{timing}."
            )
    ...
    if br == 0:
        first = f"The Mets were shut out by the {opponent_name}, {oruns}–{br}, at {venue}."
    elif largest_lead >= 2:
        first = (
            f"The Mets couldn’t hold a {largest_lead}-run lead and fell to the "
            f"{opponent_name}, {oruns}–{br}, at {venue}."
        )
    else:
        first = f"The Mets fell to the {opponent_name}, {oruns}–{br}, at {venue}."
    return first
...
def interesting_facts(boston: dict[str, Any], opponent: dict[str, Any], innings_count: int) -> list[str]:
    ...
    if boston["runs"] == 0:
        facts.append(f"The Mets were held scoreless despite putting {boston['hits']} hits on the board.")
    elif opponent["runs"] == 0:
        facts.append(f"Mets pitchers combined for a {innings_count}-inning shutout.")
    ...
    if boston["errors"] >= 2:
        ...
        facts.append(f"The Mets committed {boston['errors']} errors{detail}.")
    ...
    if top_hitter and int(top_hitter.get("hits") or 0) >= 2:
        facts.append(
            f"{top_hitter['name']} collected {top_hitter['hits']} of the Mets’ {boston['hits']} hits."
        )
    ...
    if homers:
        ...
        facts.append(f"The Mets hit {total} home run{'s' if total != 1 else ''}: {names}.")
...
def build_feed(live: dict[str, Any], content: dict[str, Any]) -> dict[str, Any]:
    ...
    boston = away if away["id"] == NYM else home
