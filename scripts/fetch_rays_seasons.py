#!/usr/bin/env python3
"""Fetch real Rays game-by-game results from the MLB Stats API.
...
TBR = 139  # MLB team id, Tampa Bay Rays
...
def war_leaders(years: list[int]) -> dict[str, list[dict]]:
    """Top three Rays players by bWAR, batters and pitchers together."""
    wanted = set(years)
...
def batting_leaders(year: int) -> dict[str, dict]:
    """Top three Rays in HR/RBI and qualified-hitter AVG/OPS."""
    pools: dict[str, list[dict]] = {}
    for pool in ("ALL", "QUALIFIED"):
        payload = fetch_json(HITTING_API.format(team=TBR, season=year, pool=pool))
...
def pitching_leaders(year: int) -> dict[str, dict]:
    """Top three Rays pitchers by WHIP, with a 40-inning minimum."""
    payload = fetch_json(PITCHING_API.format(team=TBR, season=year))
    stats = payload.get("stats", [])
...
def season_games(year: int) -> list[dict]:
    """Return completed regular-season games in order, each with the TBR view."""
    payload = fetch_json(API.format(team=TBR, season=year))
    rows, skipped = [], {}
...
def main() -> int:
    ...
    print(f"Fetching Rays seasons {years} from MLB Stats API")
    ...
    (ROOT / "data" / "rays").mkdir(parents=True, exist_ok=True)
    (ROOT / "data/rays/seasons.json").write_text(json.dumps(seasons, indent=1) + "\n")
    ...
    (ROOT / "data/rays/meta.json").write_text(json.dumps(meta, indent=1) + "\n")
    print("Wrote data/rays/seasons.json and data/rays/meta.json")
    return 0
