#!/usr/bin/env python3
"""Fetch the upcoming Yankees schedule from the MLB Stats API."""
...
OUTPUT_PATH = ROOT / "data" / "yankees" / "schedule.json"
API = (
...
def game_row(game: dict[str, Any], today, pitcher_records: dict[int, str]) -> dict[str, Any] | None:
    ...
    if away_team.get("id") == NYY:
        favorite_team, opponent, location = away, home, "away"
    elif home_team.get("id") == NYY:
        favorite_team, opponent, location = home, away, "home"
    else:
        return None
    ...
    return {
        ...
        "favorite_team_record": record(favorite_team),
        "favorite_team_pitcher": pitcher(favorite_team),
        "opponent_pitcher": pitcher(opponent),
        "favorite_team_pitcher_record": pitcher_records.get(pitcher_id(favorite_team) or 0, "—"),
...
def main() -> None:
    ...
    url = API.format(team=NYY, start=today.isoformat(), end=end.isoformat())
    ...
    print(f"Wrote {len(feed['games'])} upcoming Yankees games to {OUTPUT_PATH}")
