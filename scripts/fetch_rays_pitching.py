...
OUTPUT_PATH = ROOT / "data" / "rays" / "pitching.json"
...
def games_played(payload: dict[str, Any]) -> int:
    for record in payload.get("records", []):
        for team in record.get("teamRecords", []):
            if (team.get("team") or {}).get("id") == TBR:
                return int(team.get("wins", 0)) + int(team.get("losses", 0))
    raise RuntimeError("Could not find New York's games played in MLB standings")
...
def build_feed(
    ...
    if not actual_rows:
        raise RuntimeError("FanGraphs returned no Rays pitching rows")
        ...
        pitchers.append(
            {
                ...
                "name": actual.get("PlayerName") or "Rays pitcher",
    ...
    return {
        ...
        "method": (
            ...
            "prorated to New York's games played."
        ),
        "sources": {
...
def main() -> None:
    ...
    actual = fetch_json(ACTUAL_API.format(season=season, team=FANGRAPHS_TBR))
