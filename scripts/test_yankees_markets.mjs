...
import handler, { normalizeKalshi, normalizePoly, cleanHistory, number } from '../netlify/functions/yankees-markets.mjs';
const now = new Date('2026-09-04T20:00:00Z');
const k = { status: 'active', event_ticker: 'KXMLBGAME-26SEP041905NYYBAL', ticker: 'KXMLBGAME-26SEP041905NYYBAL-NYY', yes_bid_dollars: '0.51', yes_ask_dollars: '0.53', title: 'New York wins', volume_24h_fp: '123' };
...
assert.equal(normalizeKalshi({...k,ticker:k.ticker.replace(/NYY$/, 'BAL')},now),null);
assert.equal(number(''),null);
const e = {slug:'mlb-bal-nyy-2026-09-04',title:'Baltimore vs New York'};
const m = {id:'1',active:true,closed:false,sportsMarketType:'moneyline',question:'Baltimore vs New York',outcomes:'["Baltimore Orioles","New York Yankees"]',outcomePrices:'["0.48","0.52"]',clobTokenIds:'["10","11"]',bestBid:.47,bestAsk:.49};
...
assert.equal(normalizePoly({...e,slug:'mlb-bos-bal-2026-09-04'},m),null);
...
try {
 ...
 let r=await handler(new Request('https://test/api/yankees-markets'));let data=await r.json();
 ...
 r=await handler(new Request('https://test/api/yankees-markets'));assert.equal(r.status,502);assert.equal(r.headers.get('Cache-Control'),'no-store');
} finally {globalThis.fetch=originalFetch;}
console.log('Yankees market normalization, team isolation, outcome inversion, history, and outages: OK');
