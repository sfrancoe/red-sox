#!/usr/bin/env python3
"""Fetch National League division and Wild Card standings from MLB."""
...
OUTPUT_PATH = ROOT / "data" / "mets" / "standings.json"
API = (
...
DIVISIONS = {204: "NL East", 205: "NL Central", 203: "NL West"}
DIVISION_ORDER = {204: 0, 205: 1, 203: 2}
...
def fetch_json(url: str) -> dict[str, Any]:
    ...
    for headers in ({}, {"User-Agent": FALLBACK_USER_AGENT}):
        ...
        if headers:
            break
    raise RuntimeError(f"Could not fetch NL standings: {last_error}")
...
def build_feed(payload: dict[str, Any], season: int) -> dict[str, Any]:
    ...
    if len(divisions) != 3 or not wild_card:
        raise RuntimeError("MLB returned incomplete National League standings")
    return {
...
def main() -> None:
    ...
    url = API.format(league=NL, season=season)
    feed = build_feed(fetch_json(url), season)
    if not feed_changed(feed):
        print(f"No NL standings changes; kept {OUTPUT_PATH}")
        return
    OUTPUT_PATH.write_text(json.dumps(feed, indent=2, ensure_ascii=False) + "\n")
    print(f"Wrote NL standings for {season} to {OUTPUT_PATH}")
