#!/usr/bin/env python3
"""Validate the generated Rays feeds consumed by Hub Ball."""
...
def main() -> None:
    ...
    assert len(favorites) == 1 and favorites[0]["id"] == 139

    pitching = load("pitching")
    assert pitching["team"] == "Tampa Bay Rays"
    ...
    for source, expected_domain in domains.items():
        ...
        if source == "athletic":
            assert all(
                "rays" in f"{article['title']} {article['description']} {article['url']}".lower()
                for article in feed["articles"]
            ), source

    print("Rays feeds, team identity, and matching newspaper sources: OK")
