#!/usr/bin/env python3
"""Validate the generated Mets feeds consumed by Hub Ball."""
...
def main() -> None:
    ...
    assert len(favorites) == 1 and favorites[0]["id"] == 121

    pitching = load("pitching")
    assert pitching["team"] == "New York Mets"
    ...
    for source, expected_domain in domains.items():
        ...
        if source in {"nypost", "dailynews", "athletic"}:
            assert all(
                "mets" in f"{article['title']} {article['description']} {article['url']}".lower()
                for article in feed["articles"]
            ), source

    print("Mets feeds, team identity, and matching newspaper sources: OK")
