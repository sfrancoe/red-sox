#!/usr/bin/env python3
"""Fetch the upcoming Rays schedule from the MLB Stats API."""
...
OUTPUT_PATH = ROOT / "data" / "rays" / "schedule.json"
API = (
...
def game_row(game: dict[str, Any], today, pitcher_records: dict[int, str]) -> dict[str, Any] | None:
    ...
    if away_team.get("id") == TBR:
        favorite_team, opponent, location = away, home, "away"
    elif home_team.get("id") == TBR:
...
def main() -> None:
    ...
    url = API.format(team=TBR, start=today.isoformat(), end=end.isoformat())
    ...
    print(f"Wrote {len(feed['games'])} upcoming Rays games to {OUTPUT_PATH}")
