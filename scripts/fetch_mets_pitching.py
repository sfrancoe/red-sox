...
OUTPUT_PATH = ROOT / "data" / "mets" / "pitching.json"
...
def games_played(payload: dict[str, Any]) -> int:
    for record in payload.get("records", []):
        for team in record.get("teamRecords", []):
            if (team.get("team") or {}).get("id") == NYM:
...
def build_feed(
    ...
    if not actual_rows:
        raise RuntimeError("FanGraphs returned no Mets pitching rows")
        ...
        pitchers.append(
            {
                ...
                "name": actual.get("PlayerName") or "Mets pitcher",
