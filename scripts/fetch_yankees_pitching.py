...
OUTPUT_PATH = ROOT / "data" / "yankees" / "pitching.json"
...
def games_played(payload: dict[str, Any]) -> int:
    for record in payload.get("records", []):
        for team in record.get("teamRecords", []):
            if (team.get("team") or {}).get("id") == NYY:
...
def build_feed(
    ...
    if not actual_rows:
        raise RuntimeError("FanGraphs returned no Yankees pitching rows")
        ...
        pitchers.append(
            {
                ...
                "name": actual.get("PlayerName") or "Yankees pitcher",
