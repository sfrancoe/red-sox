#!/usr/bin/env python3
"""Fetch the four Mets newspaper feeds used by the Mets Hub app."""
...
OUTPUT_DIR = ROOT / "data" / "mets"
FALLBACK_USER_AGENT = "OpenAI File Downloader, XaiImageApiFetch/1.0"
SOURCES = {
    "nytimes": {
        "name": "The New York Times",
        "url": "https://www.nytimes.com/svc/collections/v1/publish/http%3A%2F%2Fwww.nytimes.com%2Ftopic%2Forganization%2Fnew-york-mets/rss.xml",
        "kind": "rss",
        "category": "Mets",
    },
    "nypost": {
...
def rss_articles(
    ...
    for item in root.findall(".//item"):
        ...
        if require_mets and "mets" not in f"{title} {description} {url}".lower():
            continue
...
def daily_news_articles(payload: bytes, category: str) -> list[dict[str, str]]:
    ...
    for match in anchor.finditer(page):
        ...
        if (
            ...
            or not url
            or url in seen
            or "mets" not in f"{url} {title}".lower()
        ):
            continue
        seen.add(url)
...
def ny_post_articles(payload: bytes, category: str) -> list[dict[str, str]]:
    ...
    for match in headline.finditer(page):
        url, title = clean(match.group(1)), clean(match.group(2))
        if "mets" not in f"{url} {title}".lower() or url in seen:
            continue
        seen.add(url)
...
def write_feed(key: str) -> None:
    ...
    if source["kind"] == "rss":
        articles = parser(
            payload,
            str(source["category"]),
            bool(source.get("require_mets")),
            tuple(source.get("exclude_phrases", ())),
        )
    else:
        articles = parser(payload, str(source["category"]))
    if not articles:
        raise RuntimeError(f"No {source['name']} Mets articles were found")
