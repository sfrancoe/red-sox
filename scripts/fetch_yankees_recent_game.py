#!/usr/bin/env python3
"""Fetch the most recent completed Yankees game and build a compact recap."""
...
def latest_final_game(today) -> int:
    ...
    url = SCHEDULE_API.format(team=NYY, start=start.isoformat(), end=today.isoformat())
    ...
    for date_entry in payload.get("dates", []):
        for game in date_entry.get("games", []):
            if (game.get("status") or {}).get("abstractGameState") == "Final":
                finals.append(game)
    if not finals:
        raise RuntimeError("No completed Yankees game found in the past 14 days")
...
def build_summary(boston: dict[str, Any], opponent: dict[str, Any], venue: str,
    ...
    if br > oruns:
        ...
        if walkoff:
            first = (
                ...
                f"{ordinal(int(winning_play['inning_num']))} inning as the Yankees rallied past "
                f"the {opponent_name}, {score}, at {venue}."
            )
        elif largest_deficit >= 2:
            first = (
                f"The Yankees erased a {largest_deficit}-run deficit to beat the "
                f"{opponent_name}, {score}, at {venue}."
            )
        elif oruns == 0:
            first = f"The Yankees shut out the {opponent_name}, {score}, at {venue}."
        else:
            first = f"The Yankees beat the {opponent_name}, {score}, at {venue}."
    ...
    if br == 0:
        first = f"The Yankees were shut out by the {opponent_name}, {oruns}–{br}, at {venue}."
    elif largest_lead >= 2:
        first = (
            f"The Yankees couldn’t hold a {largest_lead}-run lead and fell to the "
            f"{opponent_name}, {oruns}–{br}, at {venue}."
        )
    else:
        first = f"The Yankees fell to the {opponent_name}, {oruns}–{br}, at {venue}."
    return first
...
def interesting_facts(boston: dict[str, Any], opponent: dict[str, Any], innings_count: int) -> list[str]:
    ...
    if boston["runs"] == 0:
        facts.append(f"The Yankees were held scoreless despite putting {boston['hits']} hits on the board.")
    elif opponent["runs"] == 0:
        facts.append(f"Yankees pitchers combined for a {innings_count}-inning shutout.")
    ...
    if boston["errors"] >= 2:
        ...
        facts.append(f"The Yankees committed {boston['errors']} errors{detail}.")
    ...
    if top_hitter and int(top_hitter.get("hits") or 0) >= 2:
        facts.append(
            f"{top_hitter['name']} collected {top_hitter['hits']} of the Yankees’ {boston['hits']} hits."
        )
    ...
    if homers:
        ...
        facts.append(f"The Yankees hit {total} home run{'s' if total != 1 else ''}: {names}.")
...
def build_feed(live: dict[str, Any], content: dict[str, Any]) -> dict[str, Any]:
    ...
    boston = away if away["id"] == NYY else home
