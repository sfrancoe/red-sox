...
function gameIdentity(ticker) {
  ...
  if (!m || !m[5].includes('BOS')) return null;
  const month = months.indexOf(m[2]) + 1;
  if (!month) return null;
  const date = `20${m[1]}-${String(month).padStart(2, '0')}-${m[3]}`;
  return { date, opponent: m[5].replace('BOS', ''), time: `${m[4].slice(0,2)}:${m[4].slice(2)}` };
}
export function normalizeKalshi(m, now = new Date()) {
  ...
  const future = kind === 'KXMLB' && m.ticker === `KXMLB-${String(now.getUTCFullYear()).slice(2)}-BOS`;
  if (!game && !future) return null;
  if (kind === 'KXMLBGAME' && !m.ticker.endsWith('-BOS')) return null;
  ...
  return {
    id: m.ticker, provider: 'Kalshi', title: future ? m.title : `Boston vs ${game.opponent}`,
    question: m.title, outcome: kind === 'KXMLBGAME' ? 'Red Sox win' : (m.yes_sub_title || 'Yes'),
...
export function normalizePoly(event, m) {
  ...
  const isGame = game && [game[1],game[2]].includes('bos');
  if (!isGame && !/red sox/i.test(m.question || '')) return null;
  ...
  let index = winner ? outcomes.findIndex(x => /red sox/i.test(x)) : 0;
  ...
  return {
    ...
    outcome: winner ? 'Red Sox win' : outcomes[index],
    ...
    matchKey: isGame && winner ? `${game[3]}:${(game[1] === 'bos' ? game[2] : game[1]).toUpperCase()}` : null,
...
export const config = { path: '/api/redsox-markets', method: 'GET' };
